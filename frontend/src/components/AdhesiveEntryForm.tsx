...
import {
  ADH_CHECKED_RECEIVED_BY_OPTIONS,
  AdhesiveInwardDetail,
  AdhesiveSuggestionCategory,
  AdhesiveSuggestions,
} from "@/types/adhesive";
import AdhesiveItemFields, {
  AdhesiveItemRow,
  emptyAdhesiveItem,
  adhesiveItemRowFromItem,
  validateAdhesiveItemRow,
} from "./AdhesiveItemFields";
import { effectiveUnit, groupQuantityFor } from "./QuantityGroupSection";
import SuggestionInput from "./SuggestionInput";
import { Plus } from "lucide-react";

const SUGGESTION_FIELD: Record<AdhesiveSuggestionCategory, keyof AdhesiveSuggestions> = {
...
interface AdhesiveEntryFormProps {
  initialData?: AdhesiveInwardDetail;
  onSaved: (id: number) => void;
  onCancel?: () => void;
}

export default function AdhesiveEntryForm({ initialData, onSaved, onCancel }: AdhesiveEntryFormProps) {
  ...
  const initialCheckedByIsKnown =
    ...
    (ADH_CHECKED_RECEIVED_BY_OPTIONS as readonly string[]).includes(initialData.checked_received_by);
  ...
  const [items, setItems] = useState<AdhesiveItemRow[]>(
    initialData?.items?.length ? initialData.items.map(adhesiveItemRowFromItem) : [emptyAdhesiveItem()]
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [suggestions, setSuggestions] = useState<AdhesiveSuggestions | undefined>(undefined);

  useEffect(() => {
    api.get<AdhesiveSuggestions>("/api/adhesives/suggestions").then((res) => setSuggestions(res.data)).catch(() => {});
  }, []);

  const handleRemoveSuggestion = (category: AdhesiveSuggestionCategory, value: string) => {
    setSuggestions((prev) => {
      if (!prev) return prev;
      const field = SUGGESTION_FIELD[category];
      return { ...prev, [field]: prev[field].filter((v) => v !== value) };
    });
    api.delete(`/api/adhesives/suggestions/${category}/${encodeURIComponent(value)}`).catch(() => {});
  };

  const updateItem = (index: number, row: AdhesiveItemRow) => {
    const next = [...items];
    next[index] = row;
    setItems(next);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (items.length === 0) { setError("Add at least one Adhesive Item before saving"); return; }

    const resolvedItems = [];
    for (const row of items) {
      const result = validateAdhesiveItemRow(row);
    ...
    try {
      ...
      const res = initialData
        ? await api.put(`/api/adhesives/${initialData.id}`, payload)
        : await api.post("/api/adhesives", payload);
      onSaved(res.data.id);
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-sand rounded-2xl p-5 space-y-4">
        ...
        <div>
          ...
          <select value={checkedReceivedBy} onChange={(e) => setCheckedReceivedBy(e.target.value)} className={inputClass}>
            ...
            {ADH_CHECKED_RECEIVED_BY_OPTIONS.map((opt) => (
      ...
      <div className="space-y-3">
        <h3 className="font-semibold text-charcoal">Adhesive Items</h3>
        {items.map((row, i) => (
          <AdhesiveItemFields
            key={i}
            row={row}
        ...
        <button
          type="button"
          onClick={() => setItems([...items, emptyAdhesiveItem()])}
