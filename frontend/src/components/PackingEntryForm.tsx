...
import {
  PM_CHECKED_RECEIVED_BY_OPTIONS,
  PackingMaterialDetail,
  PackingSuggestions,
  PMSuggestionCategory,
} from "@/types/packing";
import PackingItemFields, {
  PackingItemRow,
  emptyPackingItem,
  packingItemRowFromDetail,
  validatePackingItemRow,
} from "./PackingItemFields";
import SuggestionInput from "./SuggestionInput";
import { Plus } from "lucide-react";

const SUGGESTION_FIELD: Record<PMSuggestionCategory, keyof PackingSuggestions> = {
...
interface PackingEntryFormProps {
  initialData?: PackingMaterialDetail;
  onSaved: (id: number) => void;
  onCancel?: () => void;
}

export default function PackingEntryForm({ initialData, onSaved, onCancel }: PackingEntryFormProps) {
  ...
  const initialCheckedByIsKnown =
    ...
    (PM_CHECKED_RECEIVED_BY_OPTIONS as readonly string[]).includes(initialData.checked_received_by);
  ...
  const [items, setItems] = useState<PackingItemRow[]>(
    initialData?.items?.length
      ? initialData.items.map(packingItemRowFromDetail)
      : [emptyPackingItem()]
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [suggestions, setSuggestions] = useState<PackingSuggestions | undefined>(undefined);

  useEffect(() => {
    api
      .get<PackingSuggestions>("/api/packing/suggestions")
      .then((res) => setSuggestions(res.data))
      .catch(() => {});
  }, []);

  const handleRemoveSuggestion = (category: PMSuggestionCategory, value: string) => {
    setSuggestions((prev) => {
      if (!prev) return prev;
      const field = SUGGESTION_FIELD[category];
      return { ...prev, [field]: prev[field].filter((v) => v !== value) };
    });
    api.delete(`/api/packing/suggestions/${category}/${encodeURIComponent(value)}`).catch(() => {});
  };

  const updateItem = (index: number, row: PackingItemRow) => {
    const next = [...items];
    next[index] = row;
    setItems(next);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (items.length === 0) { setError("Add at least one Packing Material item before saving"); return; }

    const resolvedItems = [];
    for (let i = 0; i < items.length; i++) {
      const result = validatePackingItemRow(items[i]);
      if (result.error) { setError(`Item ${i + 1}: ${result.error}`); return; }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-sand rounded-2xl p-5 space-y-4">
        ...
        <div>
          ...
          >
            ...
            {PM_CHECKED_RECEIVED_BY_OPTIONS.map((opt) => (
      ...
      <div className="space-y-3">
        <h3 className="font-semibold text-charcoal">Packing Material Items</h3>
        {items.map((row, i) => (
          <PackingItemFields
            key={i}
            row={row}
        ...
        <button
          type="button"
          onClick={() => setItems([...items, emptyPackingItem()])}
        ...
        >
          <Plus size={16} />
          {items.length === 0 ? "Add Item" : "Add Another Packing Material"}
        </button>
      </div>
