...
import { GenericOutwardDetail, OutwardModuleConfig } from "@/types/generic-outward";
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-taupe">This action cannot be undone.</p>
          </div>
        </div>
...
export default function GenericOutwardDetailPage({ config }: { config: OutwardModuleConfig }) {
  const { user, loading } = useAuth();
  const router = useRouter();
  const { id } = useParams<{ id: string }>();

  const [entry, setEntry] = useState<GenericOutwardDetail | null>(null);
  const [fetching, setFetching] = useState(true);
  const [showDelete, setShowDelete] = useState(false);

  const [editMode, setEditMode] = useState(false);
  const [editData, setEditData] = useState<EditHeaderState | null>(null);
  ...
  useEffect(() => {
    if (!user || !id) return;
    api.get(`${config.apiPrefix}/${id}`)
      .then((r) => setEntry(r.data))
      .catch(() => router.replace(config.routeBase))
      .finally(() => setFetching(false));
  }, [user, id, config.apiPrefix, config.routeBase, router]);

  async function handleDelete(password: string) {
    await api.delete(`${config.apiPrefix}/${id}`, { data: { password } });
    router.replace(config.routeBase);
  }
  ...
  async function handleSave() {
    ...
    setSaveError("");
    try {
      const res = await api.put(`${config.apiPrefix}/${id}`, {
        ...editData,
  ...
  const within24 = isWithin24Hours(
    toDateInput(entry.outward_date),
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title={`${config.title} Outward Entry`} backHref={config.routeBase} />
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="bg-white border border-sand rounded-2xl p-6 space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-charcoal">{config.title} Outward Entry</h2>
              <p className="text-sm text-taupe mt-0.5">
                Entry #{entry.id} · {fmtDate(entry.outward_date)}{entry.outward_time ? ` · ${fmtTime(entry.outward_time)}` : ""}
              </p>
            </div>
          ...
          {editMode && editData ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <div className="bg-cream rounded-xl px-3 py-2.5">
                ...
                <p className="text-sm font-semibold text-charcoal">{entry.items.length}</p>
              </div>
              {entry.remarks && (
          ...
          <div className="divide-y divide-sand/50">
            {entry.items.map((item, i) => (
              <div key={i} className="px-5 py-4 flex items-center justify-between gap-4">
                <p className="text-sm font-semibold text-charcoal">{item.item_name}</p>
                <p className="text-base font-bold text-rust shrink-0 whitespace-nowrap">
                  {item.quantity_issued.toLocaleString()} {item.unit}
                </p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl overflow-hidden">
            ...
            <div className="divide-y divide-amber-200/70">
              {entry.adjustments.map((adj, i) => (
                <div key={i} className="px-5 py-4 flex items-center justify-between gap-4">
                  <div>
                    <p className="text-sm font-semibold text-charcoal">{adj.item_name}</p>
                    <p className="text-xs text-amber-700 mt-0.5">{adj.reason}</p>
                  </div>
                  <p className="text-base font-bold text-amber-700 shrink-0 whitespace-nowrap">
                    +{adj.quantity.toLocaleString()} {adj.unit}
                  </p>
                </div>
              ))}
            </div>
