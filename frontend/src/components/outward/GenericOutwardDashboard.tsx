...
import GenericOutwardAnalyticsComponent from "@/components/analytics/GenericOutwardAnalytics";
import ExportModal from "@/components/ExportModal";
import api from "@/lib/api";
import { GenericOutwardListItem, OutwardModuleConfig } from "@/types/generic-outward";
import { exportGenericOutward } from "@/lib/exportUtils";
import { Download, Plus, Search } from "lucide-react";

export default function GenericOutwardDashboard({ config }: { config: OutwardModuleConfig }) {
  const { user, loading } = useAuth();
  const router = useRouter();

  const [entries, setEntries] = useState<GenericOutwardListItem[]>([]);
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(config.apiPrefix, { params });
      setEntries(res.data);
    } finally {
      setFetching(false);
    }
  }, [search, dateFrom, dateTo, config.apiPrefix]);
  ...
  const handleExport = async (format: "pdf" | "excel", df: string, dt: string, rangeLabel: string) => {
    ...
    const res = await api.get(`${config.apiPrefix}/export`, { params });
    exportGenericOutward(res.data, format, rangeLabel, `${config.title} Outward`);
  };
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title={`${config.title} Outward`} backHref="/outward" />

      <main className="max-w-6xl mx-auto px-6 py-10 space-y-6">

        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-charcoal">{config.emoji} {config.title} Outward Dashboard</h1>
            <p className="text-sm text-taupe mt-0.5">{config.title.toLowerCase()} issued for production</p>
          </div>
        ...
        <GenericOutwardAnalyticsComponent
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-cream text-left text-xs uppercase tracking-wide text-taupe">
                  ...
                  <th className="px-5 py-3 font-semibold hidden lg:table-cell">Remarks</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id}
                    onClick={() => router.push(`${config.routeBase}/${entry.id}`)}
                    ...
                    <td className="px-5 py-3 hidden sm:table-cell">
                      <div className="flex flex-wrap gap-1">
                        {entry.items.slice(0, 2).map((item, i) => (
                          <span key={i} className="text-xs bg-rust/10 text-rust rounded-full px-2 py-0.5 truncate max-w-[140px]">
                            {item.item_name}
                          </span>
                        ))}
                        {entry.items.length > 2 && (
                          <span className="text-xs text-taupe">+{entry.items.length - 2} more</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3 text-taupe hidden lg:table-cell max-w-[180px] truncate">{entry.remarks || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <ExportModal isOpen={exportOpen} onClose={() => setExportOpen(false)} moduleName={`${config.title} Outward`} onExport={handleExport} />
    </div>
  );
}
