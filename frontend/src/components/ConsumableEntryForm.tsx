...
import {
  CONS_CHECKED_RECEIVED_BY_OPTIONS,
  ConsumableInwardDetail,
  ConsumableSuggestionCategory,
  ConsumableSuggestions,
} from "@/types/consumable";
import ConsumableItemFields, {
  ConsumableItemRow,
  emptyConsumableItem,
  consumableItemRowFromItem,
  validateConsumableItemRow,
} from "./ConsumableItemFields";
import { effectiveUnit, groupQuantityFor } from "./QuantityGroupSection";
import SuggestionInput from "./SuggestionInput";
import { Plus } from "lucide-react";

const SUGGESTION_FIELD: Record<ConsumableSuggestionCategory, keyof ConsumableSuggestions> = {
...
interface ConsumableEntryFormProps {
  initialData?: ConsumableInwardDetail;
  onSaved: (id: number) => void;
  onCancel?: () => void;
}

export default function ConsumableEntryForm({ initialData, onSaved, onCancel }: ConsumableEntryFormProps) {
  ...
  const initialCheckedByIsKnown =
    ...
    (CONS_CHECKED_RECEIVED_BY_OPTIONS as readonly string[]).includes(initialData.checked_received_by);
  ...
  const [items, setItems] = useState<ConsumableItemRow[]>(
    initialData?.items?.length ? initialData.items.map(consumableItemRowFromItem) : [emptyConsumableItem()]
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [suggestions, setSuggestions] = useState<ConsumableSuggestions | undefined>(undefined);

  useEffect(() => {
    api.get<ConsumableSuggestions>("/api/consumables/suggestions").then((res) => setSuggestions(res.data)).catch(() => {});
  }, []);

  const handleRemoveSuggestion = (category: ConsumableSuggestionCategory, value: string) => {
    setSuggestions((prev) => {
      if (!prev) return prev;
      const field = SUGGESTION_FIELD[category];
      return { ...prev, [field]: prev[field].filter((v) => v !== value) };
    });
    api.delete(`/api/consumables/suggestions/${category}/${encodeURIComponent(value)}`).catch(() => {});
  };

  const updateItem = (index: number, row: ConsumableItemRow) => {
    const next = [...items];
    next[index] = row;
    setItems(next);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (items.length === 0) { setError("Add at least one Consumable Item before saving"); return; }

    const resolvedItems = [];
    for (const row of items) {
      const result = validateConsumableItemRow(row);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-sand rounded-2xl p-5 space-y-4">
        ...
        <div>
          ...
          <select value={checkedReceivedBy} onChange={(e) => setCheckedReceivedBy(e.target.value)} className={inputClass}>
            ...
            {CONS_CHECKED_RECEIVED_BY_OPTIONS.map((opt) => (
      ...
      <div className="space-y-3">
        <h3 className="font-semibold text-charcoal">Consumable Items</h3>
        {items.map((row, i) => (
          <ConsumableItemFields
            key={i}
            row={row}
            index={i}
        ...
        <button
          type="button"
          onClick={() => setItems([...items, emptyConsumableItem()])}
