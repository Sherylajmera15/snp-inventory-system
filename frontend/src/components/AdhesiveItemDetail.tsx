...
import { AdhesiveItem } from "@/types/adhesive";

interface AdhesiveItemDetailProps {
  item: AdhesiveItem;
  index: number;
}
...
export default function AdhesiveItemDetail({ item, index }: AdhesiveItemDetailProps) {
  return (
    <div className="border border-sand rounded-2xl p-5 bg-white">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-xs px-2 py-0.5 rounded-full bg-sand text-charcoal">{item.adhesive_name}</span>
      </div>
