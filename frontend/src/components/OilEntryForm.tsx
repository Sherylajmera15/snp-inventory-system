...
import {
  OIL_CHECKED_RECEIVED_BY_OPTIONS,
  OilInwardDetail,
  OilSuggestionCategory,
  OilSuggestions,
} from "@/types/oil";
import OilItemFields, {
  OilItemRow,
  emptyOilItem,
  oilItemRowFromItem,
  validateOilItemRow,
...
const SUGGESTION_FIELD: Record<OilSuggestionCategory, keyof OilSuggestions> = {
...
interface OilEntryFormProps {
  initialData?: OilInwardDetail;
  onSaved: (id: number) => void;
  onCancel?: () => void;
}

export default function OilEntryForm({ initialData, onSaved, onCancel }: OilEntryFormProps) {
  ...
  const initialCheckedByIsKnown =
    ...
    (OIL_CHECKED_RECEIVED_BY_OPTIONS as readonly string[]).includes(initialData.checked_received_by);
  ...
  const [items, setItems] = useState<OilItemRow[]>(
    initialData?.items?.length ? initialData.items.map(oilItemRowFromItem) : [emptyOilItem()]
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [suggestions, setSuggestions] = useState<OilSuggestions | undefined>(undefined);

  useEffect(() => {
    api.get<OilSuggestions>("/api/oil/suggestions").then((res) => setSuggestions(res.data)).catch(() => {});
  }, []);

  const handleRemoveSuggestion = (category: OilSuggestionCategory, value: string) => {
    setSuggestions((prev) => {
      if (!prev) return prev;
      const field = SUGGESTION_FIELD[category];
      return { ...prev, [field]: prev[field].filter((v) => v !== value) };
    });
    api.delete(`/api/oil/suggestions/${category}/${encodeURIComponent(value)}`).catch(() => {});
  };

  const updateItem = (index: number, row: OilItemRow) => {
    const next = [...items];
    next[index] = row;
    setItems(next);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (items.length === 0) { setError("Add at least one Oil / Lubricant item before saving"); return; }

    const resolvedItems = [];
    for (const row of items) {
      const result = validateOilItemRow(row);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-sand rounded-2xl p-5 space-y-4">
        ...
        <div>
          ...
          <select value={checkedReceivedBy} onChange={(e) => setCheckedReceivedBy(e.target.value)} className={inputClass}>
            ...
            {OIL_CHECKED_RECEIVED_BY_OPTIONS.map((opt) => (
      ...
      <div className="space-y-3">
        <h3 className="font-semibold text-charcoal">Oil / Lubricant Items</h3>
        {items.map((row, i) => (
          <OilItemFields
            key={i}
            row={row}
        ...
        <button
          type="button"
          onClick={() => setItems([...items, emptyOilItem()])}
        ...
        >
          <Plus size={16} />
          {items.length === 0 ? "Add Item" : "Add Another Oil / Lubricant"}
        </button>
      </div>
