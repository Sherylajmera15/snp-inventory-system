...
import ConsumableItemDetail from "@/components/ConsumableItemDetail";
import ConsumableEntryForm from "@/components/ConsumableEntryForm";
import DeleteConfirmModal from "@/components/DeleteConfirmModal";
import api from "@/lib/api";
import { ConsumableInwardDetail } from "@/types/consumable";
import { Pencil, Trash2 } from "lucide-react";
import { isWithin24Hours } from "@/components/EditProtectionModal";

function computeUnitTotals(entry: ConsumableInwardDetail): Record<string, number> {
...
export default function ConsumableDetailPage() {
  ...
  const [entry, setEntry] = useState<ConsumableInwardDetail | null>(null);
  ...
  const handleDelete = async (password: string) => {
    await api.delete(`/api/consumables/${id}`, { data: { password } });
  ...
  if (error || !entry) {
    return (
      <div className="min-h-screen bg-cream">
        <AppHeader title="Consumables Entry" backHref="/consumables" />
  ...
  if (editing) {
    return (
      <div className="min-h-screen bg-cream">
        <AppHeader title="Edit Consumables Inward Entry" backHref={`/consumables/${id}`} />
        <main className="max-w-3xl mx-auto px-6 py-10">
          <ConsumableEntryForm
            initialData={entry}
            onSaved={() => {
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Consumables Inward Entry" backHref="/consumables" />
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="space-y-4">
          <h3 className="font-semibold text-charcoal">Consumable Items ({entry.items.length})</h3>
          {entry.items.map((item, i) => (
            <ConsumableItemDetail key={item.id} item={item} index={i} />
          ))}
        </div>
