...
export default function ChemicalsDashboardPage() {
  ...
  const [stock, setStock] = useState<ChemicalStockItem[]>([]);
  ...
  const fetchStock = useCallback(async () => {
    if (!user) return;
    try {
      const res = await api.get<ChemicalStockItem[]>("/api/chemicals/stock");
      setStock(res.data);
    } catch {
      // ignore
    } finally {
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Chemicals Stock" backHref="/chemicals" />
            ...
            <div className="bg-white border border-sand rounded-2xl overflow-hidden">
              {filtered.length === 0 ? (
                <p className="text-sm text-taupe p-5">
                  {search ? "No chemicals match your search." : "No chemical stock data."}
                </p>
              ) : (
