...
export default function ChemicalsListPage() {
  ...
  const [entries, setEntries] = useState<ChemicalInwardListItem[]>([]);
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get("/api/chemicals", { params });
      setEntries(res.data);
    } finally {
      setFetching(false);
    }
  ...
  const handleExport = async (format: "pdf" | "excel", df: string, dt: string, rangeLabel: string) => {
    ...
    const res = await api.get("/api/chemicals/export", { params });
    exportChemicals(res.data, format, rangeLabel);
  };
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Chemicals" backHref="/dashboard" />
      <main className="max-w-6xl mx-auto px-6 py-10">
        <SimpleAnalytics endpoint="/api/analytics/chemicals" title="Chemicals" emoji="🧪" itemLabel="Chemical" />
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h2 className="text-xl font-semibold text-charcoal">Chemicals Inward Entries</h2>
        ...
        <div className="bg-white border border-sand rounded-2xl p-4 mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Search supplier, chemical name, manufacturer, remarks…"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-cream text-left text-taupe text-xs uppercase tracking-wide">
                  ...
                  <th className="px-5 py-3 font-medium">Chemical(s)</th>
                  <th className="px-5 py-3 font-medium">Remarks</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} onClick={() => router.push(`/chemicals/${entry.id}`)}
      ...
      <ExportModal isOpen={exportOpen} onClose={() => setExportOpen(false)} moduleName="Chemicals" onExport={handleExport} />
    </div>
  );
}
