...
import { InkOutwardDetail } from "@/types/ink-outward";
...
function fmtTime(s: string | null) { return s ? String(s).slice(0, 5) : "—"; }

function itemDisplayLabel(item: InkOutwardDetail["items"][0]): string {
  if (item.category === "Ink") {
    const c = item.color === "Spot/Pantone" && item.pantone_number
      ? `Spot/Pantone (${item.pantone_number})`
      : (item.color || "—");
    return `${item.item_type} — ${c}`;
  }
  const prefix = item.item_type === "UV Ink" ? "UV" : "Conventional";
  return `${prefix} Varnish — ${item.varnish_type || "—"}`;
}

function DeleteModal({ onConfirm, onCancel }: { onConfirm: (pw: string) => Promise<void>; onCancel: () => void }) {
  ...
  async function handleDelete() {
    ...
    try { await onConfirm(pw); }
      ...
      setError((e as { response?: { data?: { detail?: string } } })?.response?.data?.detail || "Incorrect password.");
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div><h2 className="text-sm font-bold text-charcoal">Delete Outward Entry</h2><p className="text-xs text-taupe">This action cannot be undone.</p></div>
        </div>
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 border border-sand bg-white text-charcoal rounded-xl py-2.5 text-sm font-medium hover:bg-cream transition-colors">Cancel</button>
          <button onClick={handleDelete} disabled={deleting} className="flex-1 bg-red-600 text-white rounded-xl py-2.5 text-sm font-medium hover:bg-red-700 disabled:opacity-50 transition-colors">{deleting ? "Deleting…" : "Delete"}</button>
        </div>
      </div>
    </div>
  );
...
export default function InkOutwardDetailPage() {
  const { user, loading } = useAuth();
  const router = useRouter();
  const { id } = useParams<{ id: string }>();

  const [entry, setEntry] = useState<InkOutwardDetail | null>(null);
  const [fetching, setFetching] = useState(true);
  const [showDelete, setShowDelete] = useState(false);
  const [editMode, setEditMode] = useState(false);
  const [editData, setEditData] = useState<{ outward_date: string; outward_time: string; job_name: string; job_card_number: string; issued_by: string; received_by: string; remarks: string } | null>(null);
  ...
  useEffect(() => {
    if (!user || !id) return;
    api.get(`/api/ink-outward/${id}`)
      .then((r) => setEntry(r.data))
      .catch(() => router.replace("/ink-outward"))
      .finally(() => setFetching(false));
  }, [user, id, router]);

  async function handleDelete(password: string) {
    await api.delete(`/api/ink-outward/${id}`, { data: { password } });
    router.replace("/ink-outward");
  }
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Ink & Varnishes Outward Entry" backHref="/ink-outward" />
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">

        <div className="bg-white border border-sand rounded-2xl p-6 space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-charcoal">{entry.job_name || "Ink & Varnishes Outward"}</h2>
              <p className="text-sm text-taupe mt-0.5">Entry #{entry.id} · {fmtDate(entry.outward_date)}{entry.outward_time ? ` · ${fmtTime(entry.outward_time)}` : ""}</p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              {!editMode ? (
                <>
                  ...
                  {user.role === "admin" && (
                    <button onClick={() => setShowDelete(true)} className="border border-red-200 bg-red-50 text-red-600 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-red-100 transition-colors">Delete</button>
                  )}
                </>
              ) : (
                <>
          ...
          {editMode && editData ? (
            <div className="space-y-3 pt-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-medium text-taupe mb-1">Job Name</label><input type="text" value={editData.job_name} onChange={(e) => setEditData({ ...editData, job_name: e.target.value })} className="w-full rounded-xl border border-sand bg-cream/40 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rust/30 focus:border-rust" /></div>
                <div><label className="block text-xs font-medium text-taupe mb-1">Job Card No.</label><input type="text" value={editData.job_card_number} onChange={(e) => setEditData({ ...editData, job_card_number: e.target.value })} className="w-full rounded-xl border border-sand bg-cream/40 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rust/30 focus:border-rust" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-white border border-sand rounded-2xl overflow-hidden">
          <div className="px-5 py-3 border-b border-sand bg-cream/40">
            <h3 className="text-xs font-bold text-charcoal uppercase tracking-widest">Items Issued</h3>
          </div>
          <div className="divide-y divide-sand/50">
            {entry.items.map((item, i) => (
              <div key={i} className="px-5 py-4 flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-charcoal">{itemDisplayLabel(item)}</p>
                  <p className="text-xs text-taupe mt-0.5">{item.containers_issued} containers × {item.weight_per_container} Kg/container</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-base font-bold text-rust">{item.total_weight_issued.toLocaleString()} Kg</p>
                </div>
              </div>
            ))}
          </div>
              ...
              {entry.adjustments.map((adj, i) => {
                ...
                return (
                  <div key={i} className="px-5 py-4 flex items-center justify-between gap-4">
                    ...
                    <p className="text-base font-bold text-amber-700 shrink-0">+{adj.quantity_kg.toLocaleString()} Kg</p>
                  </div>
                );
              })}
            </div>
