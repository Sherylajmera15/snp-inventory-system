...
import { MicroFilmsOutwardDetail, MICRO_ISSUED_BY_OPTIONS, MICRO_FILM_TYPES, MicroFilmType } from "@/types/micro";
...
export default function MicroFilmsOutwardDetailPage() {
  const { user, loading } = useAuth();
  const router = useRouter();
  const params = useParams();
  const id = params.id as string;

  const [entry, setEntry] = useState<MicroFilmsOutwardDetail | null>(null);
  ...
  useEffect(() => {
    if (!user) return;
    api
      .get<MicroFilmsOutwardDetail>(`/api/micro-films-outward/${id}`)
  ...
  const handleDelete = async (password: string) => {
    await api.delete(`/api/micro-films-outward/${id}`, { data: { password } });
  ...
  async function handleSave() {
    ...
    setSaveError("");
    try {
      if (!editData.job_name.trim()) { setSaveError("Job Name is required"); setSaving(false); return; }
      const res = await api.put(`/api/micro-films-outward/${id}`, {
        ...
        job_name: editData.job_name.trim(),
        ...
        film_type: (editData.film_type as MicroFilmType) || null,
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Films Outward Entry" backHref="/micro-outward" />
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        <div className="bg-white border border-sand rounded-2xl p-6 space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-charcoal">Films Outward Entry #{entry.id}</h2>
          ...
          {editMode && editData ? (
            <div className="space-y-3 pt-1">
              ...
              <div>
                <label className="block text-xs font-medium text-taupe mb-1">Quantity</label>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <Field label="Job Name" value={entry.job_name} />
              <Field label="Film Length (mm)" value={entry.film_length != null ? String(entry.film_length) : undefined} />
              <Field label="Film Width (mm)" value={entry.film_width != null ? String(entry.film_width) : undefined} />
              <Field label="Film Type" value={entry.film_type} />
              <Field label="Quantity" value={entry.quantity != null ? String(entry.quantity) : undefined} />
