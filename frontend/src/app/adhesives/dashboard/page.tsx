...
export default function AdhesivesDashboardPage() {
  const { user, loading } = useAuth();
  const router = useRouter();
  const [stock, setStock] = useState<AdhesiveStockItem[]>([]);
  ...
  const fetchStock = useCallback(async () => {
    if (!user) return;
    try {
      const res = await api.get<AdhesiveStockItem[]>("/api/adhesives/stock");
      setStock(res.data);
    } catch {
      // ignore
    } finally {
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Adhesives Stock" backHref="/adhesives" />
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by adhesive name..."
            ...
            <div className="bg-white border border-sand rounded-2xl overflow-hidden">
              {filtered.length === 0 ? (
                <p className="text-sm text-taupe p-5">
                  {search ? "No adhesives match your search." : "No adhesive stock data."}
                </p>
              ) : (
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-cream text-taupe text-xs uppercase tracking-wide">
                      <th className="px-4 py-3 text-left font-medium">Adhesive Name</th>
