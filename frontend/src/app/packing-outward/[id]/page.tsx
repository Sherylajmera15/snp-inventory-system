...
import { PackingOutwardDetail } from "@/types/packing-outward";
import { AlertTriangle, Pencil, Trash2, X } from "lucide-react";
import { isWithin24Hours } from "@/components/EditProtectionModal";

function itemLabel(material_type: string, box_size: string | null): string {
  if (material_type === "Printed Corrugated Boxes" && box_size) return `Corrugated Boxes (${box_size})`;
...
export default function PackingOutwardDetailPage() {
  const { id } = useParams<{ id: string }>();
  const { user, loading } = useAuth();
  const router = useRouter();
  const [entry, setEntry] = useState<PackingOutwardDetail | null>(null);
  ...
  useEffect(() => {
    if (!user) return;
    api.get(`/api/packing-outward/${id}`).then((r) => setEntry(r.data)).finally(() => setFetching(false));
  }, [user, id]);
  ...
  async function handleDelete() {
    ...
    try {
      await api.delete(`/api/packing-outward/${id}`, { data: { password } });
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Packing Materials Outward" backHref="/packing-outward" />
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-bold text-charcoal">📦 Outward Entry #{entry.id}</h1>
              ...
              <div className="space-y-2">
                {entry.items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between bg-cream/60 rounded-xl px-4 py-3">
                    <p className="text-sm font-medium text-charcoal">{itemLabel(item.material_type, item.box_size)}</p>
            ...
            <div className="space-y-2">
              {entry.adjustments.map((adj) => (
                <div key={adj.id} className="bg-white border border-amber-200 rounded-xl px-4 py-3 space-y-1">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-medium text-charcoal">{itemLabel(adj.material_type, adj.box_size)}</p>
