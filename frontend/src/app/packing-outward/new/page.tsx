...
import { PackingStockItem, PackingOutwardItemInput, PackingShortage } from "@/types/packing-outward";
...
function ShortageModal({ shortages, onYes, onNo }: { shortages: PackingShortage[]; onYes: () => void; onNo: () => void }) {
  return (
...
function itemKey(item: PackingOutwardItemInput) {
  return `${item.material_type}||${item.box_size ?? ""}`;
}

function itemDisplayLabel(item: PackingOutwardItemInput) {
  if (item.material_type === "Printed Corrugated Boxes" && item.box_size) return `Boxes (${item.box_size}) — ${item.quantity_issued} Boxes`;
  return `${item.material_type} — ${item.quantity_issued} ${item.unit}`;
}

function AddItemForm({ stockList, usedKeys, onAdd, onCancel }: {
  stockList: PackingStockItem[];
  usedKeys: Set<string>;
  onAdd: (item: PackingOutwardItemInput) => void;
  onCancel: () => void;
}) {
  const [filter, setFilter] = useState("");
  const [selected, setSelected] = useState<PackingStockItem | null>(null);
  const [qty, setQty] = useState<number | "">("");
  const [fifoBatches, setFifoBatches] = useState<FIFOBatch[]>([]);
  const [fifoLoading, setFifoLoading] = useState(false);

  const filtered = useMemo(() => {
    if (!filter.trim()) return stockList;
    const q = filter.toLowerCase();
    return stockList.filter((s) => s.display_label.toLowerCase().includes(q) || s.material_type.toLowerCase().includes(q));
  }, [filter, stockList]);

  const selKey = selected ? `${selected.material_type}||${selected.box_size ?? ""}` : null;
  const alreadyAdded = selKey ? usedKeys.has(selKey) : false;

  function handleAdd() {
    if (!selected || qty === "" || Number(qty) <= 0 || alreadyAdded) return;
    onAdd({ material_type: selected.material_type, box_size: selected.box_size, quantity_issued: Number(qty), unit: selected.unit });
  }
  ...
  return (
    <div className="bg-white border-2 border-rust/20 rounded-2xl p-5 space-y-4">
      ...
      <div className="max-h-52 overflow-y-auto rounded-xl border border-sand divide-y divide-sand/50">
        ...
        {filtered.map((item) => {
          const key = `${item.material_type}||${item.box_size ?? ""}`;
          ...
          return (
            <button key={key} type="button" onClick={() => {
                ...
                setFifoLoading(true);
                api.get("/api/packing-outward/stock-containers", {
                  params: { material_type: item.material_type, ...(item.box_size ? { box_size: item.box_size } : {}), unit: item.unit }
              ...
              <div>
                <p className="font-medium text-charcoal">{item.display_label}</p>
        ...
        <div className="space-y-3">
          ...
          {!fifoLoading && fifoBatches.length > 0 && (
            <FIFOPreview
              ...
              newQty={qty}
              containerLabel={
                selected.material_type === "Printed Corrugated Boxes" ? "Box Batch" :
                selected.material_type === "Sutli" ? "Bundle" : "Roll"
              }
            />
          )}
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min={0.001} step={0.001} value={qty} onWheel={(e) => e.currentTarget.blur()}
                onChange={(e) => setQty(e.target.value === "" ? "" : parseFloat(e.target.value))} placeholder="e.g. 500"
...
export default function PackingOutwardNewPage() {
  const { user, loading } = useAuth();
  const router = useRouter();
  const today = new Date().toISOString().slice(0, 10);
  const nowTime = new Date().toTimeString().slice(0, 5);

  const [stockList, setStockList] = useState<PackingStockItem[]>([]);
  const [stockLoading, setStockLoading] = useState(true);
  const [addedItems, setAddedItems] = useState<PackingOutwardItemInput[]>([]);
  const [showAdd, setShowAdd] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [shortages, setShortages] = useState<PackingShortage[]>([]);
  ...
  useEffect(() => {
    if (!user) return;
    api.get("/api/packing-outward/stock").then((r) => setStockList(r.data)).finally(() => setStockLoading(false));
  }, [user]);
  ...
  async function submitOutward(force: boolean) {
    setSubmitting(true);
    try {
      const res = await api.post("/api/packing-outward", {
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="New Packing Materials Outward" backHref="/packing-outward" />
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="space-y-3">
              ...
              {addedItems.map((item, i) => (
                <div key={i} className="bg-white border border-sand rounded-xl px-4 py-3 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <p className="text-sm text-charcoal truncate">{itemDisplayLabel(item)}</p>
                  </div>
          ...
          {showAdd && !stockLoading
            ? <AddItemForm stockList={stockList} usedKeys={usedKeys} onAdd={(item) => { setAddedItems((prev) => [...prev, item]); setShowAdd(false); }} onCancel={() => setShowAdd(false)} />
            : <button type="button" onClick={() => setShowAdd(true)} disabled={stockLoading}
                ...
                <Plus size={16} />{stockLoading ? "Loading stock…" : addedItems.length === 0 ? "Add Packing Item" : "Add Another Item"}
              </button>
          }
        </div>

        <div className="flex gap-3 pt-2">
          <button type="button" onClick={() => router.push("/packing-outward")} className="flex-1 border border-sand bg-white text-charcoal rounded-xl py-3 text-sm font-medium hover:bg-cream transition-colors">Cancel</button>
