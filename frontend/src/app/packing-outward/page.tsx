...
import PackingOutwardAnalyticsComponent from "@/components/analytics/PackingOutwardAnalytics";
import ExportModal from "@/components/ExportModal";
import api from "@/lib/api";
import { PackingOutwardListItem } from "@/types/packing-outward";
import { exportPackingOutward } from "@/lib/exportUtils";
import { Download, Plus, Search } from "lucide-react";

function itemPillLabel(item: PackingOutwardListItem["items"][0]): string {
  if (item.material_type === "Printed Corrugated Boxes" && item.box_size) return `Boxes (${item.box_size})`;
  return item.material_type;
}

export default function PackingOutwardDashboard() {
  const { user, loading } = useAuth();
  const router = useRouter();
  const [entries, setEntries] = useState<PackingOutwardListItem[]>([]);
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get("/api/packing-outward", { params });
      setEntries(res.data);
  ...
  const handleExport = async (format: "pdf" | "excel", df: string, dt: string, rangeLabel: string) => {
    ...
    const res = await api.get("/api/packing-outward/export", { params });
    exportPackingOutward(res.data, format, rangeLabel);
  };
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Packing Materials Outward" backHref="/outward" />
      <main className="max-w-6xl mx-auto px-6 py-10 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-charcoal">📦 Packing Materials Outward Dashboard</h1>
            <p className="text-sm text-taupe mt-0.5">Packing materials issued for production</p>
          </div>
          <Link href="/packing-outward/new" className="inline-flex items-center gap-2 bg-rust text-white rounded-xl px-5 py-2.5 text-sm font-semibold hover:bg-rust/90 transition-colors">
            <Plus size={16} />Create New Outward Entry
          </Link>
        </div>

        <PackingOutwardAnalyticsComponent />
        ...
        <div className="bg-white border border-sand rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Search by item, size, issued by…" value={search} onChange={(e) => setSearch(e.target.value)}
        ...
        {!fetching && entries.length === 0 && (
          <div className="bg-white border border-sand rounded-2xl p-12 text-center space-y-3">
            ...
            <Link href="/packing-outward/new" className="inline-flex items-center gap-2 bg-rust text-white rounded-lg px-4 py-2 text-sm font-medium hover:bg-rust/90 transition-colors">
        ...
        {!fetching && entries.length > 0 && (
          <div className="bg-white border border-sand rounded-2xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} onClick={() => router.push(`/packing-outward/${entry.id}`)}
                    ...
                    <td className="px-5 py-3 hidden sm:table-cell">
                      <div className="flex flex-wrap gap-1">
                        {entry.items.slice(0, 2).map((item, i) => (
                          <span key={i} className="text-xs bg-rust/10 text-rust rounded-full px-2 py-0.5 truncate max-w-[140px]">{itemPillLabel(item)}</span>
                        ))}
      ...
      <ExportModal isOpen={exportOpen} onClose={() => setExportOpen(false)} moduleName="Packing Materials Outward" onExport={handleExport} />
    </div>
  );
}
