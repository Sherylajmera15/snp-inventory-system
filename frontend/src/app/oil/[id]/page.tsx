...
import OilItemDetail from "@/components/OilItemDetail";
import OilEntryForm from "@/components/OilEntryForm";
import DeleteConfirmModal from "@/components/DeleteConfirmModal";
import api from "@/lib/api";
import { OilInwardDetail } from "@/types/oil";
...
export default function OilDetailPage() {
  ...
  const [entry, setEntry] = useState<OilInwardDetail | null>(null);
  ...
  const fetchEntry = async () => {
    setFetching(true);
    try {
      const res = await api.get(`/api/oil/${id}`);
      setEntry(res.data);
    } catch (err: any) {
  ...
  const handleDelete = async (password: string) => {
    await api.delete(`/api/oil/${id}`, { data: { password } });
    router.push("/oil");
  };
  ...
  if (error || !entry) {
    return (
      <div className="min-h-screen bg-cream">
        <AppHeader title="Oil & Lubrication Entry" backHref="/oil" />
  ...
  if (editing) {
    return (
      <div className="min-h-screen bg-cream">
        <AppHeader title="Edit Oil & Lubrication Entry" backHref={`/oil/${id}`} />
        <main className="max-w-3xl mx-auto px-6 py-10">
          <OilEntryForm
            initialData={entry}
            onSaved={() => {
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Oil & Lubrication Inward Entry" backHref="/oil" />
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="space-y-4">
          <h3 className="font-semibold text-charcoal">Oil / Lubricant Items ({entry.items.length})</h3>
          {entry.items.map((item, i) => (
            <OilItemDetail key={item.id} item={item} index={i} />
          ))}
        </div>
      </main>
