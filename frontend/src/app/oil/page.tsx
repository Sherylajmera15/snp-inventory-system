...
import { exportOil } from "@/lib/exportUtils";
import api from "@/lib/api";
import { OilInwardListItem } from "@/types/oil";
import { Download, LayoutDashboard, Plus, Search } from "lucide-react";

export default function OilListPage() {
  const { user, loading } = useAuth();
  const router = useRouter();

  const [entries, setEntries] = useState<OilInwardListItem[]>([]);
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get("/api/oil", { params });
      setEntries(res.data);
    } finally {
      setFetching(false);
    }
  ...
  const handleExport = async (format: "pdf" | "excel", df: string, dt: string, rangeLabel: string) => {
    ...
    const res = await api.get("/api/oil/export", { params });
    exportOil(res.data, format, rangeLabel);
  };
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Oil and Lubrication" backHref="/dashboard" />
      <main className="max-w-6xl mx-auto px-6 py-10">
        <SimpleAnalytics endpoint="/api/analytics/oil" title="Oil & Lubrication" emoji="🛢️" itemLabel="Oil / Lubricant" />
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h2 className="text-xl font-semibold text-charcoal">Oil & Lubrication Inward Entries</h2>
            <p className="text-sm text-taupe">All recorded oil and lubrication inward transactions.</p>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/oil/dashboard"
        ...
        <div className="bg-white border border-sand rounded-2xl p-4 mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Search supplier, oil name, manufacturer, machine, remarks…"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-cream text-left text-taupe text-xs uppercase tracking-wide">
                  ...
                  <th className="px-5 py-3 font-medium">Oil / Lubricant(s)</th>
                  <th className="px-5 py-3 font-medium">Remarks</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} onClick={() => router.push(`/oil/${entry.id}`)}
      ...
      <ExportModal isOpen={exportOpen} onClose={() => setExportOpen(false)} moduleName="Oil and Lubrication" onExport={handleExport} />
    </div>
  );
}
