...
export default function OilDashboardPage() {
  ...
  const [stock, setStock] = useState<OilStockItem[]>([]);
  ...
  const fetchStock = useCallback(async () => {
    if (!user) return;
    try {
      const res = await api.get<OilStockItem[]>("/api/oil/stock");
      setStock(res.data);
    } catch {
      // ignore
    } finally {
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Oil & Lubrication Stock" backHref="/oil" />
        ...
        {fetching ? (
          ...
          <section>
            <h2 className="text-xs font-bold text-charcoal uppercase tracking-widest mb-4">
              Oil & Lubrication Stock
            </h2>
            <div className="bg-white border border-sand rounded-2xl overflow-hidden">
              {filtered.length === 0 ? (
                <p className="text-sm text-taupe p-5">
                  {search ? "No products match your search." : "No oil or lubrication stock data."}
                </p>
              ) : (
