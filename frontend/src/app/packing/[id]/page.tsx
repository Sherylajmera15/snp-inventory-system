...
import { PackingMaterialDetail } from "@/types/packing";
...
export default function PackingDetailPage() {
  ...
  const [entry, setEntry] = useState<PackingMaterialDetail | null>(null);
  ...
  const handleDelete = async (password: string) => {
    await api.delete(`/api/packing/${id}`, { data: { password } });
    router.push("/packing");
  };
  ...
  if (error || !entry) {
    return (
      <div className="min-h-screen bg-cream">
        <AppHeader title="Packing Materials Entry" backHref="/packing" />
  ...
  if (editing) {
    return (
      <div className="min-h-screen bg-cream">
        <AppHeader title="Edit Packing Materials Entry" backHref={`/packing/${id}`} />
        <main className="max-w-3xl mx-auto px-6 py-10">
          <PackingEntryForm
            initialData={entry}
            onSaved={() => {
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Packing Materials Inward Entry" backHref="/packing" />
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="space-y-4">
          <h3 className="font-semibold text-charcoal">
            Packing Material Items ({entry.items.length})
          </h3>
          {entry.items.map((item, i) => (
            <PackingItemDetail key={item.id} item={item} index={i} />
          ))}
        </div>
      </main>
