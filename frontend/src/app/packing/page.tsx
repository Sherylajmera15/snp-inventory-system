...
import PackingAnalytics from "@/components/analytics/PackingAnalytics";
import { exportPacking } from "@/lib/exportUtils";
import api from "@/lib/api";
import { PackingMaterialListItem } from "@/types/packing";
...
export default function PackingListPage() {
  ...
  const [entries, setEntries] = useState<PackingMaterialListItem[]>([]);
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get("/api/packing", { params });
      setEntries(res.data);
    } finally {
      setFetching(false);
    }
  ...
  const handleExport = async (format: "pdf" | "excel", df: string, dt: string, rangeLabel: string) => {
    ...
    const res = await api.get("/api/packing/export", { params });
    exportPacking(res.data, format, rangeLabel);
  };
  ...
  return (
    <div className="min-h-screen bg-cream">
      <AppHeader title="Packing Materials" backHref="/dashboard" />
      <main className="max-w-6xl mx-auto px-6 py-10">
        <PackingAnalytics />
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h2 className="text-xl font-semibold text-charcoal">Packing Materials Inward Entries</h2>
            <p className="text-sm text-taupe">All recorded packing material inward transactions.</p>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/packing/dashboard"
        ...
        <div className="bg-white border border-sand rounded-2xl p-4 mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Search supplier, material type, custom name, remarks…"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-cream text-left text-taupe text-xs uppercase tracking-wide">
                  ...
                  <th className="px-5 py-3 font-medium">Material(s)</th>
                  <th className="px-5 py-3 font-medium">Remarks</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} onClick={() => router.push(`/packing/${entry.id}`)}
      ...
      <ExportModal isOpen={exportOpen} onClose={() => setExportOpen(false)} moduleName="Packing Materials" onExport={handleExport} />
    </div>
  );
}
